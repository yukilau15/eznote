...
var foundFavNotes = true; //boolean for checking user toggle view fav button
...
function initialize() {
	...
	gettingAllStorageItems.then((results) => {
		...
		for (let noteKey of noteKeys) {
			...
			//check if view fav notes is toggle
			if (curValue[2] == true) { //favourite botton is toggled
				displayNote(noteKey,curValue); //only display the fav notes
			}
		}
...
function displayNote(title, body) {
	...
	favIcon.addEventListener('click',(e)=>{
		...
		if(favIcon.textContent == 'star'){
			...
			//when user in Fav Notes page since in this page just only fav notes will showed
			if (foundFavNotes) {
				//clear the particular note
